...
export default function List() {
  ...
  return (
    <div className="flex flex-col py-6 gap-3">
      <h1 className='py-4 font-bold text-xl' style={{textAlign:'center'}}>History</h1>
      {Transactions}
    </div>
  );
}
...
function Transaction({ category, handler }) {
  ...
  return (
    <div className="item flex justify-center bg-gray-50 py-2 rounded-r" style={{ borderRight: `8px solid ${category.color ?? "#e5e5e5"}` , marginRight:'20px'}}>
      <button className='px-3' onClick={handler} style={{marginLeft:'20px'}}>
        <box-icon data-id={category._id ?? ''} color={category.color ?? "#e5e5e5"} size="15px" name="trash" ></box-icon>
      </button>
      <span className='block w-full' style={{marginLeft:'20px'}}>{category.name ?? ''}</span>
    </div>
  );
}
